...
int calculate_checksum(long long credit_card_number);
...
int main()
{
	...
	else 
	{
		...
		printf("Checksum: %d\n", checksum);
		int validity_of_checksum = validate_checksum(checksum);
		if (0 == validity_of_checksum)
		{
			printf("The checksum is invalid with its last digit not being 0.\n");
		}
		else 
		{
			printf("The checksum is valid with its last digit being 0.\n");
...
int calculate_checksum(long long credit_card_number)
{
	...
	int sum_of_eodsftl = 0;
	int sum_of_eodsftstl = 0;
	int checksum = 0;
	while (credit_card_number > 0) 
	//Loop every time the credit card number is divided by ten and greater than 0 after. In each loop, there are two paths to choose, i.e., add the last digit to the right sum.
	{
		...
		if (0 == path)
		{
			...
			sum_of_eodsftl += every_other_digit_starting_from_the_last;
			path = 1;
		}
		else
		{
			...
			if (every_other_digit_starting_from_the_second_to_last >= 10)
			{
				...
				sum_of_eodsftstl += (1 + lastdigit);
			}
			else
			{
				sum_of_eodsftstl += every_other_digit_starting_from_the_second_to_last;
			}
			path = 0;
		}
		credit_card_number /= 10;
	}
	checksum = sum_of_eodsftl + sum_of_eodsftstl;
	return checksum;
}
