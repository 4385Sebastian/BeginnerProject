...
int main()
{
	...
	int validation_of_digits = validate_digits(digits);
	//2.3 If invalid, the program ends.
	if (0 == validation_of_digits)
	{
...
int validate_digits(int digits)
{
	int validation_of_digits = 0;
	if (13 == digits || 15 == digits || 16 == digits )
	{
		validation_of_digits = 1;
	}
	return validation_of_digits;
}
...
{
	int validity_of_checksum = 0;
	int lastdigit = sum_of_every_other_digit % 10;
	if (0 == lastdigit)
	{
		validity_of_checksum = 1;
	}
	return validity_of_checksum;
}

int validate_starting_numbers(long long credit_card_number, int digits)
{
	int validation_of_starting_numbers = 0;
	int first_two_numbers = credit_card_number / pow(10, digits - 2);
	int first_number = credit_card_number / pow(10, digits - 1);
	if (15 == digits &&  (34 == first_two_numbers || 37 == first_two_numbers))
	{
		printf("Credit card brand: American Express.\n");
		validation_of_starting_numbers = 1;
	}
	if (16 == digits && 50 < first_two_numbers && 56 > first_two_numbers)
	{
		printf("Credit card brand: Mastercard.\n");
		validation_of_starting_numbers = 1;
	}
	if (13 == digits && 4 == first_number)
	{
		printf("Credit card brand: Visa.\n");
		validation_of_starting_numbers = 1;
	}
	if (16 == digits && 4 == first_number)
	{
		printf("Credit card brand: Visa.\n");
		validation_of_starting_numbers = 1;
	}
	return validation_of_starting_numbers;
}